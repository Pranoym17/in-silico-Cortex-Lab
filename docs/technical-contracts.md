...
The checked-in Phase 2 assets were generated from Nilearn `fetch_surf_fsaverage("fsaverage5")` pial surfaces. The manifest records `"source": "nilearn-fsaverage5"` and `"atlas_source": "unknown-placeholder"`.
...
These fixtures use the production filenames and JSON contract, but `mesh-manifest.json` has `"source": "dev-fixture"` and only 16 total vertices. The checked-in Phase 2 assets now replace those fixtures with real fsaverage5 geometry. Region hover/click still needs real Desikan-Killiany annotation sourcing because the current atlas file is an unknown-label placeholder.
